...
type CustomersResponse = { status: string; data: { customers: Customer[] } };
type CreatePayload = {
  name: string;
  email?: string;
  phone?: string;
...
/**
 * PUBLIC_INTERFACE
 * Customers management UI (list + create).
 */
export default function CustomersPage() {
  ...
  const create = useMutation({
    mutationFn: (payload: CreatePayload) =>
      apiFetch("/api/customers", { method: "POST", body: JSON.stringify(payload) }),
    onSuccess: async () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="Add Customer" subtitle="Create a new customer">
        <form
      ...
      <Card title="Customer List">
        {list.isLoading && <div role="status">Loading customers...</div>}
        {list.isError && (
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {list.data?.data?.customers?.map((c) => (
              ...
              {!list.isLoading && (list.data?.data?.customers?.length ?? 0) === 0 && (
                <tr>
